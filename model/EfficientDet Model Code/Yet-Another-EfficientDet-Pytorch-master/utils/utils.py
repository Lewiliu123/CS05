...
def plot_one_box(img, coord, label=None, score=None, color=None, line_thickness=None):
    ...
    if label:
        ...
        text = '{}: {:.0%}'.format(label, score)
        org = (c1[0], c1[1] - 2)
        font_scale = float(tl) / 3

        # 黑边白字
        cv2.putText(img, text, org, 0, font_scale, (0, 0, 0), thickness=tf + 2, lineType=cv2.LINE_AA)  # 黑描边
        cv2.putText(img, text, org, 0, font_scale, (255, 255, 255), thickness=tf, lineType=cv2.LINE_AA)  # 白字
